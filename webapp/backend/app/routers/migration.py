...
# --------------------------------------------------------------------------- #
# B-UML project download
# --------------------------------------------------------------------------- #

@router.get("/sessions/{session_id}/download/buml-project")
def download_buml_project(session_id: str):
    """Package the pivot model files into a single runnable B-UML project .py."""
    session = store.get(session_id)
    if session is None:
        raise HTTPException(status_code=404, detail="Session not found.")

    pivot_dir = session.pivot_dir
    domain_file = pivot_dir / "buml_model.py"
    gui_file = pivot_dir / "gui_model.py"

    if not domain_file.is_file() and not gui_file.is_file():
        raise HTTPException(status_code=404, detail="No B-UML pivot model available.")
    ...
    if domain_file.is_file():
        ...
        models_list.append("domain_model")

    if gui_file.is_file():
        if parts:
            parts.append("\n\n")
        parts.append("###############\n#  GUI MODEL  #\n###############\n\n")
        parts.append(gui_file.read_text(encoding="utf-8"))
        models_list.append("gui_model")

    project_name = "BESSER_Project"
    if session.domain_model is not None:
        raw_name = getattr(session.domain_model, "name", None)
        if raw_name:
            project_name = raw_name

    models_arg = "[" + ", ".join(models_list) + "]"
    parts.append(
        "\n\n######################\n# PROJECT DEFINITION #\n######################\n\n"
    ...
    content = "".join(parts)
    safe_name = _SAFE_NAME.sub("_", project_name) or "project"
    filename = f"{safe_name}_project.py"
